...
export default async function ({ dataDir, thoughtsDir }) {
  ...
  return Promise.all([
    createFile(dataDir, 'thoughts.json', createThoughtsMap({ thoughtFiles })),
    createFile(dataDir, 'latestThoughts.json', createThoughtsList({ limit: 10, thoughtFiles })),
    createFile(dataDir, 'thoughtsArchive.json', createThoughtsList({ thoughtFiles }))
  ])
}
